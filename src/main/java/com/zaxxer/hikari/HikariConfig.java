...
public class HikariConfig implements HikariConfigMXBean {
    ...
    // Properties changeable at runtime through the HikariConfigMXBean (可以通过 HikariConfigMXBean 运行时改变的)
    //
    private volatile String catalog;
    /**
     * 获取连接的超时时间，超过后会报SQLException，默认值为30s
     */
    private volatile long connectionTimeout;
    /**
     * 验证超时时间（connection.isVaild(validationTimeout)）
     */
    private volatile long validationTimeout;
    /**
     * 连接空闲时间，housekeeper使用
     */
    private volatile long idleTimeout;
    private volatile long leakDetectionThreshold;
    /**
     * 连接最大存活时间，超出时间后后台会对连接进行关闭，默认30min（对正在使用的连接不会立即处理）
     */
    ...
    // Properties NOT changeable at runtime (运行时不可变)
    //
    private long initializationFailTimeout;
    private String connectionInitSql;
    private String connectionTestQuery;
    /**
     * 用于指定连接池使用的DataSource的类，使用dataSourceProperties的参数变量进行辅助
     */
    ...
    /**
     * 连接池的名称，用于唯一标识一个连接池，通常作用于jmx监控和日志分析等场合。
     */
    ...
    public HikariConfig() {
        ...
        isAutoCommit = true;
        // 可以通过环境变量配置  hikaricp.configurationFile
